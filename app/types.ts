...
export interface Scenario {
    ...
    path: [number, number][];
    // Calculated station positions [lat, lng][]
    stations: [number, number][];
    stationSpacing: number; // meters
    result: ScenarioResult | null;
    createdAt: Date;
    visible: boolean;
    // AI analysis data (preserved from route selection)
    aiAnalysis?: ScenarioAIAnalysis;
}

export interface ScenarioAIAnalysis {
    // From the selected AIRouteCandidate
    candidateId?: string;
    reason?: string;
    reasoning?: string;
    keyNeighbourhoods?: string[];
    tradeoffs?: string;
    neighbourhoodImpacts?: string;
    trafficSummary?: string;
    connectivitySummary?: string;
    ridershipEstimate?: string;
    candidateScore?: number;
    // From CorridorInsights
    totalPopulationServed?: number;
    transitDesertScore?: number;
    corridorSummary?: string;
    // From analysis_summary
    analysisSummary?: string;
    // All route candidates for comparison
    allCandidates?: Array<{
        rank: number;
        name: string;
        reason: string;
