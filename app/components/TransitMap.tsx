...
import { Zone, TransitLine, Scenario, HotspotCluster, LayerVisibility } from "../types";
...
export default function TransitMap({
  ...
  useEffect(() => {
    ...
    const updateZones = async () => {
      ...
      if (!layers.needScore) return;

      zones.forEach((zone) => {
        const isSelected = selectedZone?.id === zone.id;
        // Scale radius by population density (600m–1200m)
        const radius = Math.max(600, Math.min(1200, (zone.populationDensity / 20000) * 1200));

        const circle = L.circle(zone.center, {
          radius,
          fillColor: getScoreColor(zone.needScore),
          fillOpacity: isSelected ? 0.55 : 0.3,
          color: isSelected ? "#fff" : getScoreColor(zone.needScore),
        ...
        circle.bindPopup(
          `<div style="color:#1e293b;font-size:13px;line-height:1.5">
            ...
            Need Score: <strong>${zone.needScore}/100</strong><br/>
            Pop Density: ${zone.populationDensity.toLocaleString()}/km²<br/>
            Distance to Transit: ${zone.distanceToTransit.toFixed(1)} km
          </div>`
        );

        circle.on("click", () => onZoneClickRef.current(zone));
        circle.addTo(lg);
      });
    };

    updateZones();
  }, [zones, layers.needScore, selectedZone]);
